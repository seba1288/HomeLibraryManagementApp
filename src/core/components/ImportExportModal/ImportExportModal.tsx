...
function ImportExportModal({ isOpen, onClose, onImportComplete }: ImportExportModalProps) {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Import books using the books service
      const { createBook } = await import('../../services/books/books.service');
      let imported = 0;
      let errors: string[] = [];

      for (const bookData of booksToImport) {
        try {
          // Handle both formats (from our export and custom)
          ...
          {activeTab === 'export' ? (
            <div className={styles.exportSection}>
              <p className={styles.description}>
                Export your entire library to a file. You can use this to backup your data or transfer it to another device.
              </p>
